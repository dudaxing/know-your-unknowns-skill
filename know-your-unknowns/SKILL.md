...
2. **Know the user's starting point before you calibrate to it — but do not ask for what they already told you.** Blindspot pass, teach-me and the interview all need to know their experience level, prior familiarity, and decisions already made, because that is what separates a blindspot *for them* from a generic fact. Read the request first: "I've never touched this module" or "I already decided we're using Postgres" is an anchor, and asking again reads as not having listened. Ask only for the pieces that are genuinely missing **and** would change what you produce — at most three sub-questions, in one short message, then investigate. If nothing material is missing, skip the question entirely and go straight to the territory. Never interrogate at length before delivering value.
...
3. **Investigate the territory**, having taken the starting point from the request and asked only for what was missing (principle 2). Read the actual code, git history, feature flags, and configs. If the demo-style content can't be grounded in the real project, say so and scope the investigation with the user.
