...
4. **Fold forward** — Every artifact carries an id, and the reply leads with `Artifact: KYU-EXAMPLE` so a pasted decision is attributable to *that* artifact rather than quoted, replayed, or echoed from scrollback. The agent parses whole whitelist lines and updates the plan before acting. Three checkpoints: `semantics confirmed` before porting, `Go: approve` before implementing, an agent-scored perfect `Q<n>:` quiz before merging (a pasted `Quiz score:` is never a result).
